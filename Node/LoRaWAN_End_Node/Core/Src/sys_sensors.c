...
#include "adc_if.h"		//DP: for ADC h2o sensor
#include "usart_if.h"	//DP: for USART sensor
...
#define H2O_DEFAULT_VAL			  0.69f					/*!< default soil moisture */
/* USER CODE END PD */
...
int32_t EnvSensors_Read(sensor_t *sensor_data)
{
  ...
  /* DP: REM unused
  float H2O_Value = H2O_DEFAULT_VAL;
  */
  uint16_t* N = (uint16_t*)69;
  uint16_t* P = (uint16_t*)69;
  uint16_t* K = (uint16_t*)69;

//  float soil_temp_Value = 69.0f;
#if defined (SENSOR_ENABLED) && (SENSOR_ENABLED == 1)
...
int32_t EnvSensors_Init(void)
{
  ...
  //DP: Implement USART sensor initialization here.
