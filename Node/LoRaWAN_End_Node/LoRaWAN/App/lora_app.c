...
static void SendTxData(void)
{
  ...
  if (LmHandlerIsBusy() == false)
  {
    ...
    uint32_t i = 0;
    uint16_t h2o = 0;				//DP: added capacitive moisture sensor
    uint16_t n = 0;					//DP: added NPK sensor
    uint16_t p = 0;					//DP: added NPK sensor
    uint16_t k = 0;					//DP: added NPK sensor
    uint16_t EWPT_V=0;

#endif /* CAYENNE_LPP */
    //DP: h2o = GetH2OLevel();	// Dont need because called in envsensores_read()
    ...
    // Data Packet: 13 bytes x 8-bits = 104 bits
    ...
    /*DP: DISABLE
    if  ((LmHandlerParams.ActiveRegion == LORAMAC_REGION_US915) || (LmHandlerParams.ActiveRegion == LORAMAC_REGION_AU915)
        														|| (LmHandlerParams. == LORAMAC_REGION_AS923))
    {
      AppData.Buffer[i++] = 0;
      AppData.Buffer[i++] = 0;
      AppData.Buffer[i++] = 0;
